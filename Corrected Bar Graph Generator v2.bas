Attribute VB_Name = "BarChartMacro"
Option Explicit
...
Private Sub AddPointSeries(ch As Chart, ws As Worksheet, pointHead As Long, pointRow As Long, nSer As Long, meanStart As Long, colors As Variant)
    ...
    For idx = 1 To nSer
        ...
        If n > 0 Then
            ...
            s.MarkerStyle = xlMarkerStyleCircle: s.MarkerSize = 6: s.MarkerForegroundColor = RGB(0, 0, 0): s.MarkerBackgroundColor = RGB(0, 0, 0): s.Format.Line.Visible = msoFalse
        End If
    Next idx
End Sub
...
Private Sub ApplyPublicationStyle(ByVal ch As Chart)
    Dim s As Series
    ...
    RemoveChartGridlines ch
    FormatChartTypography ch
    FormatPublicationAxes ch

    barIndex = 0
    For Each s In ch.SeriesCollection
        If IsColumnOrBarSeries(s) Then
            barIndex = barIndex + 1
            FormatPublicationBar s, CLng(colors((barIndex - 1) Mod (UBound(colors) + 1)))
        ElseIf IsScatterSeries(s) Then
            FormatPublicationMarker s
        End If
    Next s
...
Private Sub RemoveChartGridlines(ByVal ch As Chart)
    ...
    For Each axisType In Array(xlCategory, xlValue)
        For Each axisGroup In Array(xlPrimary, xlSecondary)
            Set ax = Nothing
...
Private Sub FormatChartTypography(ByVal ch As Chart)
    Dim axisType As Variant
    Dim axisGroup As Variant
    Dim ax As Axis
    ...
    For Each axisType In Array(xlCategory, xlValue)
        For Each axisGroup In Array(xlPrimary, xlSecondary)
            ...
            If Not ax Is Nothing Then
                ...
                End With
                If ax.HasTitle Then
                    With ax.AxisTitle.Format.TextFrame2.TextRange.Font
                        .Name = "Arial"
                        .Size = 16
                        .Bold = msoTrue
                        .Fill.ForeColor.RGB = RGB(0, 0, 0)
...
Private Sub FormatPublicationAxes(ByVal ch As Chart)
    Dim ax As Axis
    ...
    Set ax = ch.Axes(xlCategory, xlSecondary)
    ax.Format.Line.Visible = msoFalse
    ax.MajorTickMark = xlTickMarkNone
    ax.MinorTickMark = xlTickMarkNone
    ax.TickLabelPosition = xlNone
...
Private Sub FormatPublicationBar(ByVal s As Series, ByVal fillColor As Long)
    On Error Resume Next
    With s.Format.Fill
        .Visible = msoTrue
        .Solid
    ...
    With s.Format.Line
        ...
        .DashStyle = msoLineSolid
    End With
    If s.HasErrorBars Then
        With s.ErrorBars.Format.Line
            .Visible = msoTrue
            .ForeColor.RGB = RGB(0, 0, 0)
            .Transparency = 0
            .Weight = 2
...
Private Sub FormatPublicationMarker(ByVal s As Series)
    On Error Resume Next
...
Private Function IsColumnOrBarSeries(ByVal s As Series) As Boolean
    Select Case s.ChartType
        Case xlColumnClustered, xlColumnStacked, xlColumnStacked100, _
             xlBarClustered, xlBarStacked, xlBarStacked100
            IsColumnOrBarSeries = True
        Case Else
            IsColumnOrBarSeries = False
    End Select
End Function

Private Function IsScatterSeries(ByVal s As Series) As Boolean
    Select Case s.ChartType
